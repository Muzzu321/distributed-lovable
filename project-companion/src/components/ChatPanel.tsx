...
function MessageItem({
  ...
  // Parse raw streaming content into structured events.
  const liveEvents = useStreamParser(message.content || "");

  /*
   * Rendering logic:
   *
   * 1. If database events exist, render them.
   * 2. If the database has no MESSAGE event but has assistant content,
   *    append the plain content as a MESSAGE event.
   * 3. During live streaming, use parsed events when available.
   * 4. If there are no parsed events, render the raw assistant content
   *    as a MESSAGE event.
   */
  const eventsToRender =
      message.events && message.events.length > 0
          ? [
            ...
            ...(message.content &&
            !message.events.some(
                (event) => event.type === ChatEventType.MESSAGE
            )
                ? [
                  {
